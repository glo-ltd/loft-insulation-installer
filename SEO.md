...
1. **Per-page OG images.** Drop 1200×630 images into `public/images/og/`
   (e.g. `sheep-wool.jpg`) and set `ogImage` on the matching entry in `src/seo.js`.
   Until then every page uses `default.jpg`.

2. **Twitter handle.** If the brand has an X/Twitter account, add `twitter:site`
   (`@handle`) to the `names` object in `src/seo.js` `getMetaTags()`. Omitted for now
   rather than shipping a placeholder.

3. **Validate live:** run the Facebook Sharing Debugger and the X Card Validator on a
